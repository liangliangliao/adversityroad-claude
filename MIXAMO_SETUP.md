...
| **走路像「鞋穿反了」** | 多半是**该走路片段被镜像 / 下载成了左脚起步的镜像版本**，或模型 FBX 导入带了负缩放。排查：选中 `Walking.fbx` → Animation 页签，确认 **Mirror 未勾选**；仍不对就在 Mixamo 重新下载一个标准 `Walking`（不要 In Place 之外的镜像变体）替换。若还不对，把真机走路截图发我。 |
