...
- **角色·贰（glb）**：`Resources.Load` 按名加载与扩展名无关，`PlayerModel2.glb` 放入即被识别；
  模型没有 Animator 时运行时自动补挂。**动作库沿用角色·壹**（`Anims/`）——动作按骨骼路径
  绑定，glb 角色需为 Mixamo 标准骨架（骨骼名 mixamorig:*，如从 Blender 导出请保留骨骼名），
  骨骼名对不上时模型会保持绑定姿势不动（游戏仍可玩，换回角色·壹即恢复）。
  `PlayerModel2` 缺失时自动回退角色·壹模型。
- **武器库（glb/fbx/gltf/zip）**：放入即出现在面板中；zip 会被自动解压（只解出模型与贴图、
  跳过同名与系统垃圾文件、解压后删除压缩包）；装备时自动隐藏角色模型自带兵器、
  把新武器挂到右手、**自动归一尺寸**（包围盒最长边≈1.4m）；选「默认佩剑」恢复模型自带兵器。
  若武器在手中朝向不对，在 Unity 里把该模型拖成 Prefab 并调整子节点旋转后放回 Weapons/。
- 选择本地持久化，重启保留。
