...
def main():
    ...
    # ---- ②c 两条片段共混（相位抵消的必要条件）----
    #
    # 玩家把漂移锁定在【转圈 / 刚起步 / 转向】三个场景——它们的共同点是
    # **两条片段同时有权重**：转向是两个方向片段各半，起步变速是两个档位各半。
    # 直线匀速时只有一条主导，所以看着正常。
    # 若这三个片段的步态相位没对齐，各占一半时腿的姿态会互相抵消，读作"腿停住"。
    # blendMix = 次高权重：0 表示只有一条在演，0.5 表示两条各占一半。
    if st and "blendMix" in st[0]:
        mv2 = [r for r in st if num(r, "actual") > 0.8]
        hi = [r for r in mv2 if num(r, "blendMix") > 0.35]
        if mv2:
            print("\n【②c 两条片段共混】blendMix>0.35（两条接近各占一半）的帧占 %.1f%%"
                  % (100.0 * len(hi) / len(mv2)))
            print("     相位对齐之后，这个值高本身不再是问题；它只是用来定位"
                  "玩家说的漂移是不是发生在共混时刻。")
            if hi:
                segs2 = spans(mv2, lambda r: num(r, "blendMix") > 0.35, 0.10)
                for b in segs2[:6]:
                    print("     %.2f→%.2f (%.2fs) 速度%.1f  %s+%s" %
                          (b[0], b[1], b[1] - b[0], num(b[2], "actual"),
                           b[2].get("dir1") or "-", b[2].get("dir2") or "-"))
