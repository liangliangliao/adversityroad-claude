...
namespace AdversityRoad.Player
{
    ...
    /// ===== 该动镜的情景（只有这些）=====
    ///   | 情景 | 为什么必须动 | 怎么动 |
    ///   | **盲区 Blind**：持续行进 + 新方向看不见 | 不转就是开车看不见路（跑到墙边改朝左） | 平稳跟随，≤90°/s，大幅端更利落 |
    ///   | **敌人快出画** | 战斗里看不见对手＝失败模式 | 推回窗沿，到位即停 |
    ///   | **敌人彻底出画 0.5s** | 同上，兜底 | 无条件一次性回正 |
    ///   | **玩家显式要求** | 一键回正 / 锁定，是意图本身 | 一次有始有终的动作 |
    ///   | **机位被遮挡 0.55s** | 看不见角色 | 换角（最短驻留 4s） |
    ///   | **吊杆要穿墙** | 穿帮 | 碰撞回缩（位置，非偏航） |
    ///   | **大招/处决** | 刻意的戏剧节拍 | 推近特写（有节流） |
    ///
    /// ===== 绝不动镜的情景 =====
    ///   | 情景 | 判定 |
    ///   | **站定 Idle**（含原地转左右前后） | 没有持续位移 ⇒ 没有"前方"这回事 |
    ///   | **巡航 Cruising**：行进中但前方看得见 | 视察点在取景窗内 ⇒ 拍得到就不动 |
    ///   | **近身 Melee**（≤2.5m 时连框敌都够不着） | 战斗中稳定压倒一切 |
    ///   | **取景 ManualFraming**：玩家刚拨过镜头 | 那是他要的构图 |
    ///   | **搓杆/转圈**：方向未确认 | 那不是"要去的方向" |
    ///   | **撤离**：背离敌人全速跑 | 该看的是去路，不是身后 |
    ///
    /// 其中 Idle 与 Blind 这一对是整套设计的枢纽：**它们的几何一模一样**
    /// （镜头都不在角色背后），意图却完全相反。区分它们的不是角度，
    /// 而是**有没有真的在往某处去**——判据必须是【持续位移】，不是"碰没碰摇杆"：
    ...
    public class ThirdPersonCamera : MonoBehaviour
    {
        ...
        float _moveHoldT;                  // 「持续位移」的累计（带迟滞）
        /// <summary>要连续行进这么久才算【真的在往某处去】。
        /// 站着点一下摇杆改朝向只有一瞬的位移，撑不满它 ⇒ 仍判为「站定观察」，
        /// 镜头绝不自作主张；而一直跑动轻松满足 ⇒ 转向时该跟就跟。</summary>
        const float LocomotionHold = 0.35f;
        ...
        /// 转完停住才算。0.22s：拒掉快于 91°/s 的搓杆（点左右前后远快于此），
        /// 又把"跑到墙边改朝左"这类真实转向的延迟压到最低。</summary>
        const float DirCommitTime = 0.22f;
        ...
        void LateUpdate()
        {
            ...
            // 判据必须是【真的在往某处去】＝**持续位移**，不是"碰没碰摇杆"。
            //   · 站着点一下左/右/前/后改朝向 → 位移只有一瞬，撑不满 0.35s ⇒ 仍算站定；
            //   · 一直朝前跑 → 轻松满足 ⇒ 算行进，转向时该跟就跟。
            // 用带迟滞的累计（停下时按 1.5 倍速回退）：出招定步、落地、碎步这类
            // 短暂中断不会把行进意图打断，而真的停下来会较快回落。
            bool movingNow = moveSpeed > (player != null ? player.runSpeed : 5.2f) * 0.28f;
            _moveHoldT = Mathf.Clamp(_moveHoldT + (movingNow ? dt : -dt * 1.5f), 0f, 1f);
            bool locomotion = _moveHoldT >= LocomotionHold;
            bool idleNow = !locomotion;
            ...
            else if (autoFollow)
            {
                ...
                // 平缓度随【幅度】调：小幅纠偏最柔（0.32s），大幅掉头更利落（0.20s）。
                // 你举的"跑到墙边改朝左"正是大幅这一端——它必须实时同步，
                // 而 0.20s 下 90°/s 的目标对应峰值角加速度 ≈270°/s²，
                // 仍远在舒适区（≈600）内，平稳与跟手可以同时拿到。
                float rateSmooth = Mathf.Lerp(FollowRateSmooth, FollowRateSmoothUrgent,
                    Mathf.Clamp01((err - 45f) / 90f));
                _followRate = Mathf.SmoothDamp(_followRate, rate, ref _followRateVel,
                    rateSmooth, Mathf.Infinity, dt);
