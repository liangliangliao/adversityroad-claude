    ...
    public static class DialogueLibrary
    {
        ...
        static readonly Dictionary<string, string[]> ZoneLines = new Dictionary<string, string[]>
        {
            ...
            { "ward",   new[] { "账单，还在一张张打印。", "你什么忙也帮不上。",
                  "连这点担子都扛不动吗？" } },
            { "library", new[] { "读了这么多，还是不知道怎么活吧？", "先把这本也读完再说。",
                  "你还没想清楚，凭什么开始？" } },
            { "hall",   new[] { "为什么偏偏是你？", "如果又失败了怎么办？",
                  "先把这个问题想明白，别急着走。" } },
            { "bridge", new[] { "桥断了——你确定走得过去？", "想清楚每一步，否则就会掉下去。",
                  "还有一个问题：这一切到底有什么意义？" } },
            { "exhibit", new[] { "看清楚，这就是你——一件件失败。", "这些，你哪一件翻得了案？",
                  "站在展品前，好好看。" } },
            { "tower",  new[] { "你不行。", "你总是这样。", "别人都比你强。" } },
        };
