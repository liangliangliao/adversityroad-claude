...
## 复盘里查出来、但**没有**在代码里改掉的一件事

`MentalAttackValidator.ValidateReadability` 报了 29 条提醒，其中 25 条是同一种：
**best 比最长的干扰项明显更长**（24-2 多 17 字、18-1 多 16 字、19-2 多 14 字、19-5 多 12 字，19-1 与 24-1 各多 11 字，10-4 / 9-2 / 20-4 / 24-4 各多 10 字，其余 15 关多 6–9 字）。

这是真的破绽。增补章第 8 节自己写着"不得把 best 写得明显更长、更专业或更正能量；
三项语气长度尽量相近"——只要 best 恒定是最长的那条，玩家不读内容也能连对 90 关，
三选一就从"行为反制窗口"退回成"找最长的那一行"。选项顺序随机解决不了它：长度本身就是标记。

没有顺手改掉，是因为 PRD 第一页写着「文档性质：只增补，不删除、不替换原 PRD 既有内容」，
而这 270 句是策划的冻结文案。改它是一次**文案 pass**，该由写这批台词的人来做，
不该由实现顺手替掉。所以它被留成了一条每次构建都会打印的提醒。

需要做的是把这 25 关的 best 压到与干扰项相近的长度（通常是删掉解释性的后半句，
机制一个字都不用动）。清单在构建日志的 `[CIDIAG][三选一]` 段里，
也可以直接跑 `MentalAttackValidator.ValidateAll()` 拿到。

另外 4 条提醒是选项超过 30 字（24-1A 37 字、24-2A 36 字、18-1A 与 19-2A 各 31 字），
同属这次文案 pass。
