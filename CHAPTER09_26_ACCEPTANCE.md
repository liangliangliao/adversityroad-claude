...
## 第三轮试玩返工：这次改的是几何和文字本身，不再是翻译层

同样三条又来了一遍。前两轮我改的都是"翻译层"——把关卡表翻成蓝图的那一层，
而真正生成几何和文字的代码我一次都没打开过。这一轮去看了消费方，三条各有各的硬原因。
...
```csharp
Shell(inst, w, d, 4.2f);     // 四面到顶的实墙，只在南墙留 6 米的口
Ceiling(inst, w, d, 4.2f);   // 一整块不透光的板
```

**不管这块地有多大，净高一律 4.2 米。** 9-3 是 65×38m 的仓库，扣一个 4.2 米的盖子，
宽高比 15:1——那确实就是地下室，玩家用的这个词非常准。
我上一轮把平面尺寸改对了，可尺寸再大，封闭的盒子还是盒子。

三处一起改：

1. **净高按数据/面积给**。关卡表写了就用（9-3 高 12m、9-1 高 4m），
   没写按 `√面积 × 0.22` 推，夹在 4.5–12 米。
2. **墙分两段**：齐胸高的实墙（边界仍然清楚）＋ 上方一圈**高窗带**，只留立柱。
   视线和天光出得去，人仍然被拦住——户外的 `OpenEdge` 早就是这个思路，室内一直没跟上。
3. **顶改梁架**。只有"又矮又小"（<5m 且 <400㎡）的真·房间才封实板；
   700 ㎡ 的场地不该有不透光的天花板。

另外 `Furnish` 的工位是写死的 4×3＝12 组（桌+椅+隔板 36 件），不看面积。
32×24m 的 9-2 里就是一片插不进脚的工位田——改成按面积给（一组约占 11×11m）。

改完第 9 章的实际几何：

| 关卡 | 实建平面 | 净高 | 顶 | 墙 |
|---|---|---|---|---|
| 9-1 白纸之门 | 42×18 m | 4.0 m | 梁架（通） | 实墙 2.0 m，以上高窗带 |
| 9-2 永久校稿室 | 32×24 m | 6.1 m | 梁架（通） | 实墙 2.6 m，以上高窗带 |
| 9-3 未提交仓库 | 65×38 m | 12.0 m | 梁架（通） | 实墙 2.6 m，以上高窗带 |
| 9-4 全或无断崖 | 80×21.6 m | 9.1 m | 露天 | 护栏（通透） |
| 9-5 完美审判庭 | 55×45 m | 10.9 m | 梁架（通） | 实墙 2.6 m，以上高窗带 |

全 90 关只剩 6 关仍然封实顶（10-1、12-3、13-4、17-4、20-2、26-1），都是又矮又小的真房间。

### 二、"文字模糊看不清楚，能否固定在物体上面，不要漂浮空中"

三个成因，全在我上一轮加的 `SmallSign` 那条路上：

1. **太小**。TextMesh 的世界字高 ≈ fontSize × characterSize ÷ 10，
   `(22, 0.035)` 算出来是 **7.7 厘米**——比易拉罐上的字还小。
   这是我上一轮为了压住"满屏大字"改的，压过头了。
2. **被拉变形**。牌子挂在道具那个立方体上，而立方体带非等比缩放
   （提交台 1.6×1.2×1.0、脱离门 1.6×2.4×0.4），子物体继承这个缩放，字被横向拉三成。
   这就是"模糊"的观感来源。
3. **飘着**。挂在物体顶上 0.55 米，还跟着镜头转。

改法：新增 `OpenWorldBuilder.SurfaceSign`——**90 号字号**（图集够清晰）再用
characterSize 缩回约 22 厘米的实际字高；牌子贴在方块的侧面上、法线朝外、**不跟镜头转**；
并把道具结构改成"不缩放的根 + 缩放的子方块"，牌子挂在根上，不再被拉变形。
窄边（<0.5m）不贴——修改卡厚 14 厘米，往那个面塞三个字要缩到两成，出来是几粒斑点。

写的时候查出一个自己的朝向错误：`FaceCamera` 用的是 `LookRotation(牌子 − 镜头)`，
也就是让 **+Z 背向**镜头，字是从 **−Z 一侧**看才是正的。
我第一版写成 `LookRotation(n)`，那样每块牌子都是脸朝里的，玩家只能看到反字。已改成 `-n`。

### 三、"游戏规则不清楚，不知道如何玩"

前两轮我都在把 HUD 那一行目标行写得更准。但那一行只回答"下一步去哪"，
回答不了"这一关到底怎么玩、场上这几样东西各是干什么的"，而且一行小字压在屏幕角上，
很容易根本没被看见。

新增 `LevelBriefPanel`：**进关必看的一张卡**，读完按"开始"才进场（期间世界暂停，
否则 9-1 的白墙会在你读说明的时候一直往外推）。文案来自新数据栏 `howToPlay`。

写这几条时的硬规矩：**只描述这一关真的建出来了的东西**。
关卡表里写着、而代码还没做的机制（9-2 的边际收益递减、9-5 的四道修改门）一个字都不写——
卡上说有六张修改卡而场上没有，比不给说明更糟。所以 9-2/9-5 的卡是按现有的
【工作台】【提交台】写的，不是按 PRD 的设想写的。

每张卡最后都有一句：**"这一批关卡都不靠清怪通关。打不打得过，和过不过关是两回事。"**
这是 PRD 3.4 的硬规则，也是玩家最容易想反的一条——前 8 章都是打完就过，到这里突然不是了。

### 顺带查出的两个 bug

1. **我自己新写的净高解析是错的。** 关卡表里不少是**多组**尺度，分号隔开：
   9-2 "主厅32×24m；档案区24×18m"。按 `×` 切开取第三段拿到的是 "18m"——
   那是**下一组的进深**，不是高。9-2 和 9-5 因此都拿到了 16 米（夹上限后的值），凭空长高一倍。
   改成只认**同一组里连着三个数**的 W×D×H 写法。
2. `SiteRoom` 上有一对没人读的 `siteWidth/siteDepth`，是我前一轮把文档注释连字段一起贴错了地方。已删。

门禁那条"目标行指向核对"扩到也核对玩法说明：两处都会点名场上的东西，
点到一个不存在的名字，玩家就会满场找一块没有的牌子。离线复核 90 关，对不上 0 处。

## 第四轮：把核心思想变成玩家自己按下去的那个动作

### 〇、交互统一到【用】键，并且每一步都说清它的意义

原来是"走到 2.2 米自动触发"。那等于玩家没有做任何决定——人是走过去的，事情是自己发生的。
而这 90 关整套设计的落点正是"让玩家把判断落实成一个动作"。
**一个不需要按下去的机关，表达不了「我决定了」。**

自动触发还会直接误伤：9-3 路过目标箱就被带上了；9-1 想走近读一张修改卡，一读就等于处理掉了——
这一关唯一的那个决定被系统替玩家做了。

现在统一走这个工程既有的交互键（手机「用」，桌面 R —— E 在战斗控制器里是踢击，
沿用 `ShameInteractable` 的约定）。每次成功交互给两句：

- **反馈**：刚才发生了什么（"✔ 带上了【目标箱】"）
- **意义**：这件事在现实里叫什么（"◇ 同时搬所有箱子，等于一箱也送不出去。"）

意义句按机关类别写，十一类各一句。反馈说"系统收到了"，意义说"你刚才做的这件事是什么"——两句都要。

### 第 9 章逐关的核心思想 → 玩家动作

| 关 | 核心思想 | 玩家要做的那个动作 |
|---|---|---|
| 9-1 | 完美主义让人不开始 | 六张牌长得一样，**走近读**才分得出哪两张真挡交付；读完自己决定按不按【用】 |
| 9-2 | 边际收益递减，要主动停手 | 八处可改，每改一处可交付度涨幅**一次比一次小**；什么时候停由你决定 |
| 9-3 | 送对的那一箱，不是所有箱 | 场上还有三个【旧箱】，也搬得动，搬了不算数 |
| 9-4 | 失误只影响这一段 | 【检查点】按【用】记下进度；掉下去不算输 |
| 9-5 | 交付即胜利，不必打赢 | 走到【提交台】按【用】，Boss 当场失效 |

**9-2 过去一条循环都没有**，这一轮补齐了，因为它的核心思想恰恰和 9-1 相反：
9-1 教"别不开始"，9-2 教"别不结束"。所以它绝不能设计成"处理满 N 个就放行"——
那样停手是系统替你做的。真正要玩家做的动作是：**场上还剩着能改的东西，而你自己决定不改了，走去按提交。**
交付时按"你主动留下了多少没改"给不同的意义句，留得越多说明学得越透。

**9-3 补了三个旧箱**：原来场上只有一个箱子，"运输正确目标箱而非处理全部旧箱"这句核心机制没有落点——
只有一个箱子谈不上选对哪一箱。旧箱的牌面写"旧箱"不写"诱饵"（写了这一关当场作废），
走近那句解释也只陈述事实："上一批堆在这儿、一直没送出去的箱子。它也搬得动。"

为此加了一层 `ILevelGate`：关卡自己的通关判据由关卡自己回答。
原来提交台那个 switch 里写着 `if (levelId == 9-1) 问 9-1 的循环`，每做一关就要加一个 if，到 26 章会有十几段。

### 一、靠近就解释"这东西是干什么的"

牌面只有两三个字，那是**名字**，不是**意思**。所以补的不是又一个名字，而是它在这一关里承担什么：

- 关键物：十一类各一句解释（"这一关的终点。按下去就算交付——…"）
- 区域名牌：用这一关自己的话（终点说要做的事，中段说这一关的机制）。
  原来三句是通用套话（"阻力所在：错误策略在这里显出代价"），读完还是不知道要干什么。

停留时长按字数算（`SubtitleSeconds`：1.6 秒起步 + 每秒 5 字，夹在 3.5–12 秒），长句子自动留久一点。

### 二、文字漂浮、模糊、粗糙——查出了真正的成因

**底板在显示整张字体图集。** `WorldText` 把字的材质和底板的材质放在同一张
`_byColor` 表里，而 `Bind()` 会把字体图集贴到表里**每一个**材质上——底板也在表里。
于是每块底板显示的是一整张糊满字形的透明贴图。

截图里"装车月台"背后那块发白、发糊、像脏了一样的板子就是它。
**粗糙的不是字，是字底下那块板。** 分成 `_glyphMats` / `_solidMats` 两张表，底板这一路再也不会被 `Bind` 碰到。

另外区域名牌原来是**悬在 3.4~4.2 米、跟着镜头转的一块字**——"漂浮"三个字指的就是它。
现在建成立柱 + 牌面的实体，落到地面上，牌高压到 1.9 米（人眼高度），字刷在牌面上、不跟镜头转。

### 顺带修好的两个门禁问题

- `csmissing` 不认识**以 `;` 收尾的声明**（接口成员、abstract 方法）。本仓库第一次出现接口
  （`ILevelGate`）就撞上了，把两条声明报成"找不到声明"。已教会它认接口体和 abstract 修饰。
  修的时候自己又写了个 bug：用 `id(code)` 当缓存键——id 在对象回收后会被重用，
  于是这个文件拿到的是**另一个文件**的接口区间。已去掉缓存。回插验证：仍能抓出真实的漏方法。
- 指向核对门禁把 `【用】` 报成"场上没有这块牌子"。`【】` 里也可能是**按键名**
  （"按【用】/ R" 是本工程既有写法），已把按键名单独放行。离线复核 90 关：指向对不上 0 处。

## 第五轮：查出"所有东西堆在门口"的根因；场地放大 2.3 倍；木牌刻字

### 最重要的一条：关键物全被摆到了入口，整块场地空着没用

`InternalProps.Build(lv, root, origin, spawn, **exitPoint**)`——而 `exitPoint` 是
**回城的门**，它就在玩家落点身后 2.5 米：

```csharp
inst.playerSpawn = FindClearSpawn(inst, d);                 // 场地南端 -d/2+4
inst.exitPoint   = inst.playerSpawn - up*1.1 - forward*2.5; // 落点身后 2.5 米
```

于是"从落点走向终点"这条方向算出来是**朝着入口往回走**：
提交台被摆在落点身后四米，其余关键物沿那个方向一路铺到场地外面去。

实机看到的就是"一进关，所有东西都堆在脚边"——上一张截图里目标箱和装车月台
挤在出生点正前方，正是这个。**场地一直是够大的，只是全部内容都挤在门口那一小块。**
玩家五轮都在说"空间狭窄、移动困难"，有一大半是这么来的，而我前四轮一直在改场地尺寸。

修法：`SiteInstance` 增加 `farPoint`（场地另一端，`origin + (0, 1.1, d*0.34)`），
关键物、9-1 的白墙外推方向、9-2 的卡片铺设一律改用它。

### 一、场地放大

关卡表里的米数是**灰盒最小可用尺度**，不是一块让人走得开的场地。按 1.5 倍放，
区间从 `20-80 × 15-64` 抬到 `46-120 × 34-96`：

| 关卡 | 旧 | 新 | 倍数 |
|---|---|---|---|
| 9-1 白纸之门 | 42×18 = 756 ㎡ | 63×34 = 2142 ㎡ | ×2.8 |
| 9-2 永久校稿室 | 32×24 = 768 ㎡ | 48×36 = 1728 ㎡ | ×2.2 |
| 9-3 未提交仓库 | 65×38 = 2470 ㎡ | 98×57 = 5558 ㎡ | ×2.2 |
| 9-4 全或无断崖 | 80×21.6 = 1728 ㎡ | 120×34 = 4080 ㎡ | ×2.4 |
| 9-5 完美审判庭 | 55×45 = 2475 ㎡ | 82×67.5 = 5569 ㎡ | ×2.2 |

全 79 关（写了尺寸的）平均 2712 ㎡ → 6241 ㎡。杂物改成按每 500 ㎡ 一件给，
不再是固定条数——固定条数会把小场地塞满、大场地又显空。

### 二、"布局类似化、高度重合"

90 关里 55 关落在 `hall`，而 `BuildHall` 每次建的是同一个东西：3×3 减中心＝八根柱子，
加一块永远在 +0.22d 的台子。这 55 关走进去当然全是一个样。

现在按 seed 在四种柱网里挑：双排列柱（中间一条通道）／环形柱列（中心空出一大块）／
四角柱（最开阔）／单侧列柱＋矮隔断（偏心，最不像盒子）。台子的位置和尺寸跟着变体走。
seed 取自 levelId 的哈希，所以同一关每次进去一样（空间记忆还在），不同关之间才不一样。

### 三、"所有物体就是些简单的方形图形"

这条说得完全对：整个 `SiteBuilder` 里只出现过 `PrimitiveType.Cube` 一种图元，
"desk" 就是一个 2.2×1×1.1 的实心方块——桌子和箱子长得一模一样。

加了 `Shape()` / `Cyl()` 两个构件函数，把常用道具改成**多件拼出来的东西**：

| 道具 | 原来 | 现在 |
|---|---|---|
| 桌子 | 1 个实心方块 | 台面 + 四条圆腿 + 显示器支臂 |
| 椅子 | 1 个实心方块 | 坐面 + 靠背 + 四条圆腿 |
| 货架 | 1 个实心方块 | 四根立柱 + 四层层板（**看得穿**，不再挡视线挡路） |
| 柱子 | 方柱 | 圆柱 + 上下柱础 |
| 长椅 | 1 块板 | 三条木板 + 两副铁腿 |
| 盆栽 | 方盆 + 方叶 | 圆盆 + 胶囊枝干 + 球形树冠 |
| 管道 | 一根长方体 | 圆柱 + 两端法兰 |
| 木箱 | 1 个方块 | 箱体 + 两道棱边压条 |

这是纯代码生成的场景，没有任何美术资源；能做的是把"一堆方块"变成"能看出是什么的东西"，
但它到不了写实。这一条我说在前面。

### 四、文字刻在实体木牌上

- **区域名牌**：两根**圆木立柱** + 三块横木板（板缝看得见）+ 上下压条包边。
- **关键物**：正反两面各钉一块小木牌，四角有球形铆钉。
- **刻字**：新增 `CarvedSign`——字色取**比木板更深的木纹色**（像凿掉一层露出的暗面），
  紧贴板面只让 1.5 厘米，并且**不配深色底板**——那块底板正是"贴了张标签"观感的来源。
  真正的刻痕要改网格，纯代码生成做不到；这里做的是"看上去是刻的"。

## 第六轮：把战斗和机关咬在一起（"和战斗完全脱节"）

你选的三条是：**心理概念太抽象、动作太单薄、和战斗完全脱节**。第三条是根，另外两条跟着它一起解。

### 根因：PRD 早就写好了敌人行为，我一条都没实现

关卡表的"敌人/干扰"栏本来写得非常具体：

| 关 | 关卡表原文 | 我之前做成了什么 |
|---|---|---|
| 9-1 | 白纸哨兵×2，主要制造错误提示与打断 | 两只通用小怪站着 |
| 9-2 | 校稿幽灵 / 反复修改节点 | 两只通用小怪站着 |
| 9-3 | 最后检查者×3 **会把目标箱推回检查区** | 三只通用小怪站着 |
| 9-4 | 全或无刽子手会破坏部分路线但始终留可继续路径 | 一只通用小怪站着 |

**一条都没实现。** 于是"不以清怪定义胜利"被我做成了"战斗根本没有意义"。
这两件事差得很远：前者是**赢的方式不是清场**，后者是**打架被从关卡里摘了出去**。

正确的关系是：敌人**妨碍你完成那个动作**。你打它们不是为了清场，
是为了挣出把事情做完的那点空间和时间。这样战斗系统重新有用，而胜利条件仍然不是清怪。

### 三关的战斗 × 机关

**9-1 白纸哨兵：在你动笔之前，它打不倒。**
砍多少下都只跳"护体"，直到你走到【工作台】按【用】做出第一版——
那一刻两只哨兵同时变得能被打倒，白墙也不再往外推。
"完美主义不让你开始"从此不是一个数字，是一个身体经验：**先动手，路才打得开。**

**9-2 校稿幽灵：每多打磨一处不挡交付的地方，屋里就多进来一只。**
这一条替掉了我上一版那个"可交付度 +7（共 35），上一次 +22，在往下掉"——
那是文档语言，玩家在读一条曲线，不是在体会它。
现在收益越来越小、而屋里的人越来越多，"再改下去不划算"不需要任何一句说明。
阻断项不招幽灵：处理它们正是这一关要你做的事，不该有惩罚。
最坏情况 2+6＝8 只，但同时参战仍由 `SiteEncounterDirector` 压在 3 个以内。

**9-3 最后检查者：它把你手上的箱子推回检查区。**
这一条是 PRD 自己写的设计。它把这一关从"走过去按一下"变成
**护着一件东西穿过一段有人拦你的路**：被贴上就前功尽弃，
所以要先把它们引开、打退，或者挑空档走。
打倒它们能让路好走，但通关判定仍然是那一箱有没有上车。

### 顺带修掉的一个真 bug（这一条影响所有关卡）

写敌人战术时顺手核对了关键物落位，发现
`InternalProps.Build(..., spawn, **exitPoint**)` 里的 `exitPoint` 是**回城的门**，
就在玩家落点身后 2.5 米。于是"从落点走向终点"这条方向算出来是**朝入口往回走**，
提交台被摆在落点身后四米，其余关键物一路铺到场地外面。
实机就是"一进关所有东西都堆在脚边，整块场地空着没用"。
已改为新增的 `farPoint`（场地另一端）。
