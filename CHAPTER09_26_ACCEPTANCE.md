...
### 编译红了一次：lint 全绿，但代码根本编译不过

我把 `InternalProp.ResetSession()`（声明在 MonoBehaviour 上）写成了
`InternalProps.ResetSession()`（静态工具类）。两个类只差一个字母，还住在同一个文件里。
这是 CS0117，而当时 11 条 linter 全绿——**这正是最坏的一种情况：门禁说没事，玩家那边编译失败**。

顺带修掉同一支里的两个运行时问题：目标箱的 `Use` 把 `_used` 置回了 false，
而箱子带上之后就贴在玩家身边、距离恒小于 2.2m，于是每帧重跑一次，字幕一秒刷几十行；
走近提示也会对着手上的箱子每 8 秒念一遍。

补了第 12 条门禁 `tools/csstatic.py`：受体本身就是类型名时（`Foo.Bar`），
Foo 若是本仓库里唯一声明、且无基类的类型，Bar 就必须在 Foo 上——不需要任何作用域分析。
`csmember` 只认 `List<T> _x` 这一种受体，看不到静态受体，两条互补。

初版一跑出来 16 处"可疑"，全是误报：成员的**类型**可以带空格
（`Action<float, float>`、`(string label, int count)`），而正则的字符类不含空格，
于是 `GameEvents` 的六个事件、`FailureLog` 的三个元组方法全被判成不存在。
按 `csmember` 抬头那句"会喊狼来了的 linter 比没有更糟"，改成按括号深度扫而不是靠正则猜形状，
全库 0 误报，并且能抓出这次原样的错。
